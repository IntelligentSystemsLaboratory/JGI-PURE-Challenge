...
namespace AVPTM
{
    ...
    public class LDAShared : ILDA
    {
            ...
            out Dirichlet[] postTheta, out Dirichlet[] postPhi)
        {
            ...
            if (Math.Abs(numDocsPerBatch) < double.Epsilon) numDocsPerBatch = 1;
            var boundary = new int[NumBatches+1];
            boundary[0] = 0;
            var currBoundary = 0.0;
            ...
            try
            {
                for (int pass = 0; pass < NumPasses; pass++)
                {
                    ...
                    for (int batch = 0; batch < NumBatches; batch++)
                    {
                        ...
                        if (pass == 0)
                        {
                            ThetaInit.ObservedValue = LDA.GetInitialisation(numDocsInThisBatch, NumTopics, ThetaSparsity);
                        }
                        else
                        {
                        ...
                        var numWordsInDocBatch = new int[numDocsInThisBatch];
                        var wordsInDocBatch = new int[numDocsInThisBatch][];
                        var wordCountsInDocBatch = new double[numDocsInThisBatch][];
        ...
        /// <param name="sparsity">Sparsity level</param>
        /// <returns>Dirichlet array</returns>
        private static DirichletArray CreateUniformDirichletArray(
            int length, int valueLength, Sparsity sparsity)
        {
            var result = new Dirichlet[length];
            for (int i = 0; i < length; i++)
            {
                result[i] = Dirichlet.Uniform(valueLength, sparsity);
            }

            return (DirichletArray)Distribution<Vector>.Array(result);
        }
    }
}
